    ...
    <div className={`space-y-6 ${className}`}>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button 
                ...
                className="text-xs font-semibold text-slate-400 hover:text-[#0079CD] transition-colors cursor-pointer flex items-center gap-1"
              >
      ...
      {showAllCountriesModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setShowAllCountriesModal(false)}></div>
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[85vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                <Globe className="w-5 h-5 text-[#0079CD]" />
                All Visitor Locations
              </h3>
              <button onClick={() => setShowAllCountriesModal(false)} className="text-slate-400 hover:text-slate-600 transition-colors bg-white rounded-full p-1.5 shadow-sm border border-slate-100">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
              </button>
            </div>
            
            <div className="p-5 overflow-y-auto overflow-x-hidden flex-1 scrollbar-thin">
              {sessionsByCountry.length === 0 ? (
                <div className="py-8 text-center text-slate-400 font-medium">No location data available yet</div>
              ) : (
                <div className="space-y-4">
                  {sessionsByCountry.map((loc, idx) => {
                    const pct = Math.round((loc.sessions / totalSessionsCount) * 100);
                    return (
                      <div key={idx} className="space-y-1.5">
                        <div className="flex justify-between text-sm font-semibold">
                          <span className="flex items-center gap-2 text-slate-700">
                            <span className="text-lg leading-none">{getCountryFlag(loc.country)}</span>
                            <span>{getCountryFullName(loc.country)}</span>
                          </span>
                          <span className="text-slate-500 font-mono">{loc.sessions} sessions ({pct}%)</span>
                        </div>
                        <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                          <div style={{ width: `${Math.min(100, Math.max(2, pct))}%` }} className="bg-[#214A9E] h-full rounded-full transition-all duration-500"></div>
                        </div>
                      </div>
                    );
                  })}
