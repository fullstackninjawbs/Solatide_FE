...
const CustomAddressAutocomplete = ({ value, onChange, onSelect }) => {
  ...
  const fetchAddresses = async (q) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&addressdetails=1&limit=10`);
