...
const ProductInfoSection = ({ product }) => {
    const hasOverview = !!product.overviewHtml;
    const hasApplications = !!product.researchApplicationsHtml;
    const hasSpecs = (product.technicalSpecsTable && product.technicalSpecsTable.length > 0) || product.technicalSpecs?.rawHtml || product.technicalSpecsRawHtml;

    if (!hasOverview && !hasApplications && !hasSpecs) {
        return null;
    }
